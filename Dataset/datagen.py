...
if __name__ == '__main__':
    datagen = ImageDataGenerator(rotation_range=30,  width_shift_range=0.3,  height_shift_range=0.3, shear_range=0.3, zoom_range=0.2, horizontal_flip=True, fill_mode='constant')
    data_files = os.listdir(covid_std_path)
    data = np.zeros(shape=(len(data_files), 299, 299, 3))
    data_index = 0
