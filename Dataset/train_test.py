...
from config import train_path, test_path, covid_path_name

main_read_path = train_path
main_write_path = train_path

def splitFileData(arr_x, arr_y):
    x_train, x_test, y_train, y_test = train_test_split(arr_x, arr_y, test_size=0.40, random_state=42)
    print(len(x_train))
    print(len(x_test))
...
if __name__ == '__main__':
    ...
    data_files = os.listdir(os.path.join(train_path, covid_path_name))
    data_y = np.zeros(len(data_files))
    data_file_train, data_file_test = splitFileData(data_files, data_y)

    moveFiles(data_file_test, os.path.join(train_path, covid_path_name), os.path.join(test_path, covid_path_name))
